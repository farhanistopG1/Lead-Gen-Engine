...
# Timing (faster since no API costs)
...
MAX_RETRIES = 5
BASE_BACKOFF = 10
MAX_HTML_LENGTH = 8000  # Keep full analysis capability
MIN_HTML_LENGTH = 500

# ============================================================================
# OLLAMA API FUNCTIONS
# ============================================================================
def ask_ollama(prompt, max_tokens=800, temperature=0.1):
    """
    Call local Ollama - NO COST, NO LIMITS!
    """
    try:
...
# ============================================================================
# SAFE SHEET OPERATIONS (SAME AS BEFORE)
# ============================================================================
...
# ============================================================================
# NORMALIZATION & DUPLICATE DETECTION (SAME AS BEFORE)
# ============================================================================
...
# ============================================================================
# MAIN PROCESSING (OLLAMA VERSION)
# ============================================================================
def process_single_lead():
    ...
    for idx, lead in enumerate(all_leads):
        ...
        if status == "pending":
            ...
            # OLLAMA ANALYSIS (SAME QUALITY AS GEMINI, BUT FREE!)
            try:
                print(f"🤖 Starting Ollama analysis (FREE!)...")
                
                # SAME PROMPT AS GEMINI VERSION
                analysis_prompt = f"""Analyze the website for "{restaurant_name}" and provide a comprehensive analysis.

Please provide:

1. KEY INFORMATION (3-4 bullet points):
   - What the business does
   - Contact information found/missing
   - Main issues or problems

2. FIX CHECKLIST (5-7 actionable items):
   - Missing contact details (email, phone)
   - Broken or unclear elements
...
{cleaned_html}"""
                ...
                # Call Ollama (FREE, UNLIMITED!)
                full_response = ask_ollama(
                    analysis_prompt,
                    max_tokens=800,
                    temperature=0.1
                )
                ...
                print(f"   ✅ Ollama completed in {api_time:.1f}s")
                print(f"   📊 ~{input_tokens:,} in + ~{output_tokens:,} out = ~{total_tokens:,} tokens")
                print(f"   💰 Cost: ₹0 (FREE!)")
                print(f"   🚀 No rate limits, no quotas!")
...
# ============================================================================
print("\n" + "="*70)
print("🚀 OLLAMA Lead Processor - FREE UNLIMITED VERSION")
print("="*70)
print(f"💎 Model: {OLLAMA_MODEL} (Local)")
print(f"🧹 HTML Cleaning: 90%+ reduction")
print(f"⚡ Ollama calls: 1 per lead (FREE!)")
print(f"📊 Daily Limit: {MAX_LEADS_PER_DAY} leads")
print(f"⏱️  Delay: {MIN_DELAY_SECONDS}-{MAX_DELAY_SECONDS} seconds")
print(f"💰 Cost: ₹0 FOREVER!")
print(f"🚀 Process unlimited leads - no API costs!")
print("="*70 + "\n")
